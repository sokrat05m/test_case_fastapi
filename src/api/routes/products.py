...
from api.schemas import ProductBaseSchema, MinMaxSumSchema
from api.service import get_products, min_max_sum, get_product, get_products_by_category

product_router = APIRouter(prefix='/products')


@product_router.get("/", response_model=list[ProductBaseSchema])
...
@product_router.get("/{product_id}", response_model=ProductBaseSchema)
async def get_product_by_id(product_id: int, db: SessionDep):
    product = get_product(db=db, product_id=product_id)
    return product


@product_router.get("/category/{category_id}", response_model=list[ProductBaseSchema])
